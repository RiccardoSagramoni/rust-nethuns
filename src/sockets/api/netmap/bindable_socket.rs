...
impl<State: RcState> BindableNethunsSocketInnerTrait<State> for BindableNethunsSocketNetmap<State> {
    fn bind(
        mut self,
        dev: &str,
        queue: NethunsQueue,
    ...
    > {
        ...
        let mut free_ring =
            CircularBuffer::new(nm_port_d.reg.nr_extra_bufs as _, &|| 0);
