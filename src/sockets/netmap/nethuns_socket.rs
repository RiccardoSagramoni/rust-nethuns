...
use crate::misc::{nethuns_dev_queue_name, CircularBuffer};
...
pub(crate) struct NethunsSocketNetmap {
    ...
    free_ring: Option<CircularBuffer<u32>>,
}
...
impl NethunsSocket for NethunsSocketNetmap {
    ...
    fn bind(
        ...
        // TODO comment
        self.free_ring =
            Some(CircularBuffer::new(nm_port_d.reg.nr_extra_bufs as _, &|| 0));
        ...
        if self.rx() {
            while scan != 0 {
                self.free_ring.as_mut().unwrap().push(scan);
                scan = unsafe {
                    let ptr = netmap_buf(&some_ring, scan as _) as *const u32;
                    assert!(!ptr.is_null());
                    ptr.read_unaligned()
    ...
    fn recv(&mut self) -> Result<RecvPacket, NethunsRecvError> {
        ...
        if self.free_ring.as_ref().unwrap().empty() {
        ...
        cur_netmap_slot.buf_idx = self.free_ring.as_mut().unwrap().pop();
...
impl Drop for NethunsSocketNetmap {
    fn drop(&mut self) {
        ...
        while !self.free_ring.as_ref().unwrap().empty() {
            let idx = self.free_ring.as_mut().unwrap().pop();
            let next = netmap_buf(some_ring, idx as _) as *mut u32;
